...
public class NeoVmTest {
    ...
    public void sendTransaction() throws Exception {
        ...
        if(true){
            Transaction tx = tstSdk.vm().makeDeployCodeTransaction(codeHex, true, "name",
                    "v1.0", "author", "email", "desp", account.getAddressU160().toBase58(),30000000,0);
            tstSdk.signTx(tx, new Account[][]{{account}});
            Object result = tstSdk.getConnect().sendRawTransaction(tx.toHexString());
            Thread.sleep(6000);
            Object obj = tstSdk.getConnect().getContract(codeAddress);
            assertTrue(obj != null);
        }
        if(true){
            ...
            func.name = name;
            func.setParamsValue();
            Object obj =  tstSdk.neovm().sendTransaction(Helper.reverse(codeAddress),null,null,0,0,func, true);

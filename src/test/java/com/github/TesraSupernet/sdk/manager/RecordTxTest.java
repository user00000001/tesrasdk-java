//package com.github.TesraSupernet.sdk.manager;
//
//import com.alibaba.fastjson.JSON;
//import com.alibaba.fastjson.JSONObject;
//import com.alibaba.fastjson.util.IOUtils;
//import com.github.TesraSupernet.TstSdk;
//import com.github.TesraSupernet.common.Common;
//import com.github.TesraSupernet.common.Helper;
//import com.github.TesraSupernet.core.VmType;
//import com.github.TesraSupernet.core.transaction.Transaction;
//import com.github.TesraSupernet.sdk.wallet.Account;
//import com.github.TesraSupernet.sdk.wallet.Identity;
//import org.junit.Assert;
//import org.junit.Before;
//import org.junit.Test;
//
//
//public class RecordTxTest {
//
//    TstSdk tstSdk;
//    String codeAddress = "806256c36653d4091a3511d308aac5c414b2a444";
//    String password = "111111";
//    String claimId = "7f2075f54450432e7582b58088f269d778206be3af08c650af35a61d894a6d35";
//
//    @Before
//    public void setUp() throws Exception {
//
//        tstSdk = TstSdk.getInstance();
//        String restUrl = "http://127.0.0.1:25770";
//        String codeHex = "";
//        tstSdk.setRestful(restUrl);
//        tstSdk.setDefaultConnect(tstSdk.getRestful());
//        tstSdk.openWalletFile("RecordTxTest.json");
//        tstSdk.neovm().claimRecord().setCodeAddress(codeAddress);
//        Identity id = tstSdk.getWalletMgr().createIdentity(password);
//
//        tstSdk.vm().setCodeAddress(codeAddress);
//
//        Transaction tx = tstSdk.vm().makeDeployCodeTransaction(codeHex, true, "name", "1.0", "1", "1", "1", VmType.NEOVM.value(),id.tstid,0);
//        tstSdk.signTx(tx,id.tstid,password);
//        String txHex = Helper.toHexString(tx.toArray());
//        tstSdk.getConnectMgr().sendRawTransaction(txHex);
//        Thread.sleep(6000);
//    }
//
//    @Test
//    public void sendCommit() throws Exception {
//        Account payer = tstSdk.getWalletMgr().createAccount(password);
//        Identity identity = tstSdk.getWalletMgr().createIdentity(password);
//        tstSdk.nativevm().tstId().sendRegister(identity,password,payer.address,password,0);
//
//        Thread.sleep(6000);
//
//        String commitRes = tstSdk.neovm().claimRecord().sendCommit(identity.tstid,password,identity.tstid,claimId,0);
//        Assert.assertNotNull(commitRes);
//        Thread.sleep(6000);
//
//        String getstatusRes = tstSdk.neovm().claimRecord().sendGetStatus(identity.tstid,password,claimId);
//        Assert.assertTrue(getstatusRes.contains("0"));
//
//        String revokeRes = tstSdk.neovm().claimRecord().sendRevoke(identity.tstid,password,claimId,0);
//        Assert.assertNotNull(revokeRes);
//        Thread.sleep(6000);
//
//        String getstatusRes2 = tstSdk.neovm().claimRecord().sendGetStatus(identity.tstid,password,claimId);
//        Assert.assertTrue(getstatusRes2.contains("1"));
//
//    }
//}
...
import com.github.TesraSupernet.crypto.*;
import com.github.TesraSupernet.sdk.exception.*;
...
import com.github.TesraSupernet.sdk.wallet.Control;
...
public class WalletMgr {
    ...
    public Identity importIdentity(String label,String encryptedPrikey, String password,byte[] salt, String address) throws Exception {
        String prikey = com.github.TesraSupernet.account.Account.getGcmDecodedPrivateKey(encryptedPrikey, password, address,salt, walletFile.getScrypt().getN(), scheme);
    ...
    public IdentityInfo getIdentityInfo(String ontid, String password,byte[] salt) throws Exception {
        com.github.TesraSupernet.account.Account acct = getAccountByAddressOrOntId(ontid, password,salt);
    ...
    private AccountInfo createAccountInfo(String label,String password,byte[] salt, byte[] prikey) throws Exception {
        com.github.TesraSupernet.account.Account acct = createAccount(label,password,salt, prikey, true);
    ...
    public String privateKeyToWif(String privateKey) throws Exception {
        com.github.TesraSupernet.account.Account act = new com.github.TesraSupernet.account.Account(Helper.hexToBytes(privateKey), scheme);
        return act.exportWif();
    }
    public com.github.TesraSupernet.account.Account getAccount(String address, String password) throws Exception {
        return getAccount(address, password,getWallet().getAccount(address).getSalt());
    }
    public com.github.TesraSupernet.account.Account getAccount(String addressOrOntId, String password,byte[] salt) throws Exception {
    ...
    public AccountInfo getAccountInfo(String addressOrOntId, String password,byte[] salt) throws Exception {
        AccountInfo info = new AccountInfo();
        com.github.TesraSupernet.account.Account acc = getAccountByAddressOrOntId(addressOrOntId, password,salt);
    ...
    private com.github.TesraSupernet.account.Account createAccount(String label, String password, byte[] salt,byte[] privateKey, boolean accountFlag) throws Exception {
        com.github.TesraSupernet.account.Account account = new com.github.TesraSupernet.account.Account(privateKey, scheme);
        Account acct;
        switch (scheme) {
    ...
    private com.github.TesraSupernet.account.Account getAccountByAddressOrOntId(String addressOrOntId,String password,byte[] salt) throws Exception {
        try {
            if(addressOrOntId.startsWith(Common.didont)){
                ...
                for (Account e : walletInMem.getAccounts()) {
                    if (e.address.equals(addressOrOntId)) {
                        String prikey = com.github.TesraSupernet.account.Account.getGcmDecodedPrivateKey(e.key, password, e.address,salt, walletFile.getScrypt().getN(), scheme);
                        return new com.github.TesraSupernet.account.Account(Helper.hexToBytes(prikey), scheme);
                    }
                }
            }

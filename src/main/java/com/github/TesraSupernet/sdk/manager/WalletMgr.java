...
public class WalletMgr {
    ...
    public Identity importIdentity(String label,String encryptedPrikey, String password,byte[] salt, String address) throws Exception {
        ...
        prikey = null;
        return getWallet().getIdentity(info.tstid);
    }
    ...
    public com.github.TesraSupernet.account.Account getAccount(String addressOrTstId, String password,byte[] salt) throws Exception {
        return getAccountByAddressOrTstId(addressOrTstId, password,salt);
    }
    ...
    private com.github.TesraSupernet.account.Account createAccount(String label, String password, byte[] salt,byte[] privateKey, boolean accountFlag) throws Exception {
        ...
        if (accountFlag) {
            ...
            if (walletInMem.getIdentities().size() == 0) {
                idt.isDefault = true;
                walletInMem.setDefaultTstid(idt.tstid);
            }
    ...
    private com.github.TesraSupernet.account.Account getAccountByAddressOrTstId(String addressOrTstId,String password,byte[] salt) throws Exception {
        try {
            if(addressOrTstId.startsWith(Common.didtst)){
                ...
                for (Account e : walletInMem.getAccounts()) {
                    if (e.address.equals(addressOrTstId)) {

...
public class Wallet implements Cloneable {
    private String name = "com.github.TesraSupernet";
    private String version = "1.0";
    private String createTime = "";
    private String defaultTstid = "";
    ...
    public String getDefaultTstid() {
        return defaultTstid;
    }

    public void setDefaultTstid(String defaultTstid) {
        this.defaultTstid = defaultTstid;
    }
    ...
    public boolean removeIdentity(String tstid) {
        for (Identity e : identities) {
            if (e.tstid.equals(tstid)) {
    ...
    public Identity getIdentity(String tstid) {
        for (Identity e : identities) {
            if (e.tstid.equals(tstid)) {
                return e;
            }
    ...
    public void setDefaultIdentity(int index) throws Exception {
        ...
        defaultTstid = identities.get(index).tstid;
    }

    public void setDefaultIdentity(String tstid) {
        for (Identity e : identities) {
            if (e.tstid.equals(tstid)) {
                e.isDefault = true;
                defaultTstid = tstid;
    ...
    private Identity addIdentity(String tstid) {
    ...
    private void addIdentity(Identity idt) {
        for (Identity e : getIdentities()) {
            if (e.tstid.equals(idt.tstid)) {
                return;
            }
        }
        getIdentities().add(idt);
    }
    public Identity addTstIdController(String tstid, String key, String id,String pubkey) {
        Identity identity = getIdentity(tstid);
        if (identity == null) {
            identity = addIdentity(tstid);
        }

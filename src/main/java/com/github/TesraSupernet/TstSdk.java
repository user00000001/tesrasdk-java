...
import com.github.TesraSupernet.sdk.manager.*;
import com.github.TesraSupernet.sidechain.SidechainVm;
import com.github.TesraSupernet.smartcontract.NativeVm;
import com.github.TesraSupernet.smartcontract.NeoVm;
import com.github.TesraSupernet.smartcontract.Vm;
import com.github.TesraSupernet.smartcontract.WasmVm;
...
public class TstSdk {
    ...
    private static TstSdk instance = null;
    ...
    public static synchronized TstSdk getInstance(){
        if(instance == null){
            instance = new TstSdk();
        }
        return instance;
    ...
    public Transaction signTx(Transaction tx, String addressOrTstId, String password,byte[] salt) throws Exception{
        signTx(tx, new Account[][]{{getWalletMgr().getAccount(addressOrTstId, password,salt)}});
        return tx;
    }
    /**
     * sign tx
    ...
    public byte[] signatureData(com.github.TesraSupernet.account.Account acct, byte[] data) throws SDKException {
    ...
    public boolean verifySignature(byte[] pubkey, byte[] data, byte[] signature) throws SDKException {
        ...
        try {
            ...
            data = Digest.sha256(Digest.sha256(data));
            return sign.verifySignature(new com.github.TesraSupernet.account.Account(false, pubkey), data, signature);
        } catch (Exception e) {
            throw new SDKException(e);
        }
    }
    ...
    private Transaction makeNativeTransaction(String contractHash, List paramList, String payer, long gasLimit, long gasPrice) {
        try {
            String TST = "0100000000000000000000000000000000000000";
            String TSG = "0200000000000000000000000000000000000000";
            String TSTID = "0300000000000000000000000000000000000000";
            String method = new String((byte[]) paramList.get(0));
            List list = new ArrayList();
            Struct struct = new Struct();
            struct.list = (List) paramList.get(1);
            if (contractHash.equals(TSG) || contractHash.equals(TSG)) {
            ...
            }else if(contractHash.equals(TSTID)){
    ...
    public Map parseTransaction(String txhexstr) throws SDKException {
        ...
        try {
            ...
            if (tx instanceof DeployCode) {
                ...
                if (codeHexStr.length() > 44 && codeHexStr.substring(codeHexStr.length() - 44, codeHexStr.length()).equals(Helper.toHexString("Tesra.Native.Invoke".getBytes()))) {
